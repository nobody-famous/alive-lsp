...
(defun xyz-read-text-token (&key state token-type predicate)
    (loop :for ch := (look-ahead state)

          :while (and ch (funcall predicate ch))
          :do (next-char state)

          :finally (return (xyz-new-token state token-type))))
...
(defun xyz-read-ws-token (state)
    (xyz-read-text-token :state state
                         :token-type types:*ws*
                         :predicate (lambda (ch)
                                        (and ch (is-ws ch)))))
...
(defun xyz-read-symbol-token (state)
    (xyz-read-text-token :state state
                         :token-type types:*symbol*
                         :predicate (lambda (ch)
                                        (not (or (is-delim ch)
...
(defun xyz-read-ch-token (state token-type)
    (next-char state)
    (xyz-new-token state token-type))
...
(defun xyz-read-string-token (state)
    (let ((start (file-position (input state))))
        ...
        (let ((end (file-position (input state))))
            ...
            (xyz-new-token state types:*string*))))
...
(defun xyz-read-comment-token (state)
    (xyz-read-text-token :state state
                         :token-type types:*line-comment*
                         :predicate (lambda (ch)
                                        (and ch
...
(defun xyz-read-colons-token (state)
    (xyz-read-text-token :state state
                         :token-type types:*colons*
                         :predicate (lambda (ch)
                                        (and ch
...
(defun xyz-read-ifdef-token (state sign-ch)
    (next-char state)
    ...
    (labels ((check-ifdef (text)
        ...
        (loop :with str := (make-string-output-stream)
              ...
              :finally (return (let ((text (format nil "#~A~A" sign-ch (get-output-stream-string str))))
                                   (if (check-ifdef text)
                                       (xyz-new-token state types:*ifdef-true* text)
                                       (xyz-new-token state types:*ifdef-false* text)))))))
...
(defun xyz-read-block-comment-token (state)
    (next-char state)
    ...
    (loop :with depth := 0
          ...
          :finally (return (xyz-new-token state types:*block-comment*))))
...
(defun xyz-read-macro-generic-token (state)
    (xyz-read-text-token :state state
                         :token-type types:*macro*
                         :predicate (lambda (ch)
                                        (and ch
...
(defun xyz-read-macro-char (state)
    (next-char state)
    ...
    (let ((ch (look-ahead state)))
        ...
        (xyz-new-token state types:*macro*)))
...
(defun xyz-read-sharp-quote (state)
    (next-char state)
    (xyz-new-token state types:*macro*))
...
(defun xyz-read-macro-token (state)
    (next-char state)

    (let ((ch (look-ahead state)))
        (if ch
            (cond ((or (char= ch #\+)
                       (char= ch #\-)) (xyz-read-ifdef-token state ch))
                  ((char= ch #\|) (xyz-read-block-comment-token state))
                  ((char= ch #\\) (xyz-read-macro-char state))
                  ((char= ch #\') (xyz-read-sharp-quote state))
                  (t (xyz-read-macro-generic-token state)))
            (xyz-new-token state types:*macro*))))
...
(defun xyz-read-comma-token (state)
    (next-char state)
...
(defun xyz-read-vertical-bar-token (state)
    (next-char state)
    ...
    (loop :with ch := (look-ahead state)
          ...
          :finally (progn (when (and ch (char= ch #\|))
                                (next-char state))
                          (return (xyz-new-token state types:*symbol*)))))
...
(defun xyz-next-token (state)
    ...
    (let ((ch (look-ahead state)))
        (when ch
              (cond ((char= ch #\() (xyz-read-ch-token state types:*open-paren*))
                    ((char= ch #\)) (xyz-read-ch-token state types:*close-paren*))
                    ((char= ch #\') (xyz-read-ch-token state types:*quote*))
                    ((char= ch #\`) (xyz-read-ch-token state types:*back-quote*))
                    ((char= ch #\") (xyz-read-string-token state))
                    ((char= ch #\;) (xyz-read-comment-token state))
                    ((char= ch #\:) (xyz-read-colons-token state))
                    ((char= ch #\#) (xyz-read-macro-token state))
                    ((char= ch #\,) (xyz-read-comma-token state))
                    ((char= ch #\|) (xyz-read-vertical-bar-token state))
                    ((is-ws ch) (xyz-read-ws-token state))
                    (T (xyz-read-symbol-token state))))))
