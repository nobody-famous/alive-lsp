...
(defun get-message-handlers (state)
    (list (cons "initialize" (lambda (deps msg) (declare (ignore deps)) (alive/session/handler/init:request msg)))
          ...
          (cons "$/alive/eval" (lambda (deps msg)
                                   (threads:xyz-run-in-thread deps state (or (cdr (assoc :method msg)) "eval")
                                                              (cdr (assoc :id msg))
                                                              (lambda ()
                                                                  (alive/session/handler/eval:handle deps state msg)))))
          (cons "$/alive/evalInFrame" (lambda (deps msg)
                                          (threads:xyz-run-in-thread deps state (or (cdr (assoc :method msg)) "eval-in-frame")
                                                                     (cdr (assoc :id msg))
                                                                     (lambda ()
                                                                         (alive/session/handler/eval:handle-in-frame deps state msg)))))
          ...
          (cons "$/alive/traceFunction" (lambda (deps msg)
                                            (threads:xyz-run-in-thread deps state (or (cdr (assoc :method msg)) "traceFunction")
                                                                       (cdr (assoc :id msg))
                                                                       (lambda ()
                                                                           (alive/session/handler/traced-fns:trace-fn deps state msg)))))
          (cons "$/alive/untraceFunction" (lambda (deps msg)
                                              (threads:xyz-run-in-thread deps state (or (cdr (assoc :method msg)) "untraceFunction")
                                                                         (cdr (assoc :id msg))
                                                                         (lambda ()
          ...
          (cons "$/alive/tracePackage" (lambda (deps msg)
                                           (threads:xyz-run-in-thread deps state (or (cdr (assoc :method msg)) "tracePackage")
                                                                      (cdr (assoc :id msg))
                                                                      (lambda ()
                                                                          (alive/session/handler/traced-fns:trace-pkg deps msg)))))
          (cons "$/alive/untracePackage" (lambda (deps msg)
                                             (threads:xyz-run-in-thread deps state (or (cdr (assoc :method msg)) "untracePackage")
                                                                        (cdr (assoc :id msg))
                                                                        (lambda ()
          ...
          (cons "$/alive/loadAsdfSystem" (lambda (deps msg)
                                             (threads:xyz-run-in-thread deps state (or (cdr (assoc :method msg)) "ASDF")
                                                                        (cdr (assoc :id msg))
                                                                        (lambda ()
          ...
          (cons "$/alive/compile" (lambda (deps msg)
                                      (threads:xyz-run-in-thread deps state (or (cdr (assoc :method msg)) "Compile")
                                                                 (cdr (assoc :id msg))
                                                                 (lambda ()
                                                                     (alive/session/handler/compile:file deps state msg)))))
          (cons "$/alive/loadFile" (lambda (deps msg)
                                       (threads:xyz-run-in-thread deps state (or (cdr (assoc :method msg)) "Load File")
                                                                  (cdr (assoc :id msg))
                                                                  (lambda ()
          ...
          (cons "$/alive/inspect" (lambda (deps msg)
                                      (threads:xyz-run-in-thread deps state (or (cdr (assoc :method msg)) "Inspect")
                                                                 (cdr (assoc :id msg))
                                                                 (lambda ()
          ...
          (cons "$/alive/inspectEval" (lambda (deps msg) (threads:xyz-run-in-thread deps state (or (cdr (assoc :method msg)) "Inspect")
                                                                                    (cdr (assoc :id msg))
                                                                                    (lambda ()
...
(defun create-deps (&key input-stream output-stream state)
    (let ((handlers (get-message-handlers state)))
        (deps:create :msg-handler (lambda (deps msg) (alive/session/message:handle deps state handlers msg))
                     ...
                     :try-compile (lambda (path) (alive/file:xyz-try-compile path))
                     :do-compile (lambda (path &key stdin-fn stdout-fn stderr-fn) (alive/file:xyz-do-compile path :stdin-fn stdin-fn :stdout-fn stdout-fn :stderr-fn stderr-fn))
                     :do-load (lambda (path &key stdin-fn stdout-fn stderr-fn) (alive/file:xyz-do-load path :stdin-fn stdin-fn :stdout-fn stdout-fn :stderr-fn stderr-fn)))))
