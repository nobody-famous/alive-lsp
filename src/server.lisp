...
(defparameter *new-message-handlers*
              (list (cons "initialize" (lambda (deps state msg) (declare (ignore deps state)) (alive/session/handler/init:request msg)))
                    ...
                    (cons "textDocument/completion" (lambda (deps state msg) (declare (ignore deps state)) (alive/session/handler/document:completion msg)))
                    ...
                    (cons "textDocument/onTypeFormatting" (lambda (deps state msg) (declare (ignore deps state)) (alive/session/handler/document:on-type msg)))
                    (cons "textDocument/rangeFormatting" (lambda (deps state msg) (declare (ignore deps state)) (alive/session/handler/document:formatting msg)))
                    (cons "textDocument/selectionRange" (lambda (deps state msg) (declare (ignore deps state)) (alive/session/handler/document:selection msg)))
                    (cons "textDocument/semanticTokens/full" (lambda (deps state msg) (declare (ignore deps state)) (alive/session/handler/document:sem-tokens msg)))
                    ...
                    (cons "$/alive/listThreads" (lambda (deps state msg) (declare (ignore state)) (alive/session/handler/threads:new-list-all deps msg)))
                    ...
                    (cons "$/alive/compile" (lambda (deps state msg) (declare (ignore deps state))
                                                (threads:run-in-thread (or (cdr (assoc :method msg)) "Compile")
                                                                       (cdr (assoc :id msg))
                                                                       (lambda ()
                                                                           (alive/session/handler/compile:file msg)))))
                    (cons "$/alive/loadFile" (lambda (deps state msg) (declare (ignore deps state))
                                                 (threads:run-in-thread (or (cdr (assoc :method msg)) "Load File")
                                                                        (cdr (assoc :id msg))
                                                                        (lambda ()
                                                                            (alive/session/handler/compile:load-file msg)))))

                    (cons "$/alive/symbol" (lambda (deps state msg) (declare (ignore deps state)) (alive/session/handler/symbol:for-pos msg)))
                    ...
                    (cons "$/alive/inspect" (lambda (deps state msg) (declare (ignore deps state))
                                                (threads:run-in-thread (or (cdr (assoc :method msg)) "Inspect")
                                                                       (cdr (assoc :id msg))
                                                                       (lambda ()
                                                                           (alive/session/handler/inspect:do-inspect msg)))))
                    ...
                    (cons "$/alive/inspectSymbol" (lambda (deps state msg) (declare (ignore deps state))
                                                      (threads:run-in-thread (or (cdr (assoc :method msg)) "Inspect")
                                                                             (cdr (assoc :id msg))
                                                                             (lambda ()
                                                                                 (alive/session/handler/inspect:do-symbol msg)))))
